...
from method.audio_dc_base import AUDIO_DC_BASE
from model import get_model
from utils.clustering_sample_selection import Clustering_Sample_Selection as CSS

class DualCondensation_Audio(AUDIO_DC_BASE):
    ...
    async def audio_cnn_spatial(self, x):
        return x.permute(0, 2, 1)     # B X L X C
    
    async def audio_cnn_semantic(self, x):
        return x.mean(-1)               # B x C
    
    async def hubert_spatial(self, x):
        return x                       # B x L x C
    
    async def hubert_semantic(self, x):
        return x.mean(1)               # B x C
    ...
    async def _condense(self):
        ...
        with torch.no_grad():
            ...
            dummy_input = torch.zeros(2, 1, self.max_length).to(self.device)

            # Pre-compiling to prevent possible error
            _, feature_1 = self.net_1(dummy_input)
            _, feature_2 = self.net_2(dummy_input)
            
            if 'hubert' in self.model.lower():
                self.spatial_1 = self.bert_spatial
                self.sementic_1 = self.bert_semantic
            else:
                self.spatial_1 = self.audio_cnn_spatial
                self.sementic_1 = self.audio_cnn_semantic
            if 'hubert' in self.model_2.lower():
                self.spatial_2 = self.hubert_spatial
                self.sementic_2 = self.hubert_semantic
            else:
                self.spatial_2 = self.audio_cnn_spatial
                self.sementic_2 = self.audio_cnn_semantic

            feature_1_shapes = torch.stack([torch.tensor((await self.spatial_1(f)).shape) for f in feature_1]) # L x B x N x C
            feature_2_shapes = torch.stack([torch.tensor((await self.spatial_2(f)).shape) for f in feature_2]) # L x B x N x C
        ...
        for it in range(self.start_iteration, self.iteration):
            ...
            for o in range(self.o_iter):
                ...
                for c in range(self.global_rank, self.num_classes, self.world_size):
                    ...
                    if o % self.interval == 0:
                        with torch.no_grad():
                            ...
                            dist = dist[torch.arange(len(dist)), pred1]+1e-8
                            mean_dist1 = dist.mean() / dist.max()
                            query_list[c] = q_idxs1.detach().cpu()
                            ...
                            dist = dist[torch.arange(len(dist)), pred2]+1e-8
                            mean_dist2 = dist.mean() / dist.max()
                    ...
                    # model1_scale = model2_dist[c] / (model1_dist[c] + model2_dist[c]+1e-8)
                    # model2_scale = model1_dist[c] / (model1_dist[c] + model2_dist[c]+1e-8)
                ...
                with torch.no_grad():
                    grad = self.synthetic_images.grad.view(self.num_classes, self.n_ipc, self.channel, self.max_length)
                    grad = grad / (grad.abs().max(dim=2, keepdim=True)[0] + 1e-8)
                    grad = grad.view(self.num_classes, self.n_ipc, self.channel, self.max_length)
