...
from datasets.esc10 import ESC10
from datasets.imdb import IMDB
from torchvision import datasets, transforms
from scipy.ndimage.interpolation import rotate as scipyrotate
from model._convnet import MLP, ConvNet, LeNet, AlexNet, VGG11BN, VGG11, ResNet18, ResNet18BN_AP
...
def get_dataset(dataset, data_path):
    if dataset == 'MNIST':
        ...
        transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])
        dst_train = TinyImageNet(data_path, train=True, download=True, transform=transform) # no augmentation
        dst_test = TinyImageNet(data_path, train=False, download=True, transform=transform)
        class_names = dst_train.classes
    elif dataset == 'ESC50':
        channel = 1
        im_size = 80000
        ...
        std = None
        transform = None
        dst_train = ESC50(data_path, download=True, train=True, transform=transform)
        dst_test = ESC50(data_path, download=True, train=False, transform=transform)
        class_names = dst_train.classes
    elif dataset == 'ESC10':
        channel = 1
        im_size = 80000
        ...
        std = None
        transform = None
        dst_train = ESC10(data_path, download=True, train=True, transform=transform)
        dst_test = ESC10(data_path, download=True, train=False, transform=transform)
        class_names = dst_train.classes
    elif dataset == 'IMDB':
        channel = 1
        im_size = 64
        ...
        dst_train = IMDB(data_path, tokenizer, im_size, download=True, train=True, transform=transform)
        dst_test = IMDB(data_path, tokenizer, im_size, download=True, train=False, transform=transform)
        class_names = dst_train.classes
    else:
...
def augment(images, dc_aug_param, device):
    ...
    if dc_aug_param != None and dc_aug_param['strategy'] != 'none':
        ...
        def scalefun(i):
            ...
            tmp = F.interpolate(images[i:i + 1], [h, w], mode='bilinear')
...
def DiffAugment(x, strategy='', seed = None, param = None):
    if seed is None:
        param.Siamese = False
    else:
        param.Siamese = True
